...
--- Commands
ESX.RegisterCommand("mieszkania:odswiez", Config.AllowedGroups, function(xPlayer)
  PropertiesRefresh()
end, false, {help = TranslateCap("refresh_desc")})

ESX.RegisterCommand("mieszkania:stworz", "user", function(xPlayer)
...
ESX.RegisterCommand("mieszkania:adminMenu", Config.AllowedGroups, function(xPlayer)
...
ESX.RegisterServerCallback("esx_property:attemptSellToPlayer", function(source, cb, PropertyId, PlayerId)
  ...
  if xTarget and (xTarget.getAccount("bank").money >= Price) and (xPlayer.job.name == PM.job) then
    ...
    Properties[PropertyId].Owned = true
    TriggerClientEvent("esx_property:syncProperties", -1, Properties)
    --TriggerEvent('hp-logs:sendLog', source, '[SPRZEDAŁ POSIADŁOŚĆ DLA GRACZA]','\n**Gracz: **'..GetPlayerName(PlayerId)..'\n**Kupił od: **'..GetPlayerName(GetPlayerName(source))..'\n**Posiadłość: **'..Properties[PropertyId].Name..'\n**Za kwotę: **'..ESX.Math.GroupDigits(Price), nazwaWebhooka)
    if Config.OxInventory then
    ...
    if PM.Enabled then
      ...
      local SocietyPrice = math.percent(60, Price)
      TriggerEvent('esx_addonaccount:getSharedAccount', PM.society, function(account)
        account.addMoney(math.floor(SocietyPrice))
      end)
      xPlayer.addAccountMoney("bank", PlayerPrice, "Sold Property")

      TriggerEvent('hp-logs:sendLog', source, '[ZAKUPIŁ POSIADŁOŚĆ]','\n**Gracz: **'..GetPlayerName(xTarget.source)..'\n**Zakupił posiadłość: **'..Properties[PropertyId].Name..'\n**Sprzedawca: **'..GetPlayerName(xPlayer.source)..'\n**Za kwotę: **'..ESX.Math.GroupDigits(Price)..'\n**Rodzaj zapłaty: ** BANK'..'\n**Zyska dla firmy: **'..tonumber(SocietyPrice), 'property_zakupił_dom')
...
ESX.RegisterServerCallback("esx_property:buyFurniture", function(source, cb, PropertyId, PropName, PropIndex, PropCatagory, pos, heading)
  ...
  TriggerEvent('hp-logs:sendLog', xPlayer.source, '[KUPIŁ MEBEL]',"\n**Gracz: **"..GetPlayerName(xPlayer.source)..'\n**Kupił mebel do domu: **'..Properties[PropertyId].Name..'\n**Nazwa propa: **'..Config.FurnitureCatagories[PropCatagory][PropIndex].title..'\n**Cena: **'..tostring(Config.FurnitureCatagories[PropCatagory][PropIndex].price)..'\n**Opłacone: **'..xPlayer.getAccount("bank").money >= Config.FurnitureCatagories[PropCatagory][PropIndex].price and "Yes" or "No", 'property_kupil_mebel')
end)
