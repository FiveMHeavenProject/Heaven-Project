...
RegisterServerEvent('esx_billing:sendMechanicBill')
AddEventHandler('esx_billing:sendMechanicBill', function(senderIdentifier ,identifier, sharedAccountName, label, amount)
	amount = ESX.Math.Round(amount)
	...
	if amount > 0 then
		TriggerEvent('esx_addonaccount:getSharedAccount', sharedAccountName, function(account)
			if account then
				MySQL.insert('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (?, ?, ?, ?, ?, ?)', {identifier, senderIdentifier, 'society', sharedAccountName, label, amount},
				function(rowsChanged)
					-- identifier.showNotification(TranslateCap('received_invoice'))
				end)
			else
				MySQL.insert('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (?, ?, ?, ?, ?, ?)', {identifier, senderIdentifier, 'player', senderIdentifier, label, amount},
				function(rowsChanged)
					-- identifier.showNotification(TranslateCap('received_invoice'))
				end)
			end
