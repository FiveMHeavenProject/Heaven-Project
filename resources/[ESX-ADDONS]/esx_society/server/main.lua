...
AddEventHandler('esx_society:withdrawMoney', function(societyName, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
		if amount > 0 and account.money >= amount then
			...
			TriggerEvent('hp-logs:sendLog', source, '[BOSSMENU - WYPŁATA GOTÓWKI]',"\n**Gracz: **"..GetPlayerName(source)..'\n**Wypłacił pieniądze. Ilość: **'..amount..'\n**Firma: **'..xPlayer.job.name..'\n**Stan konta po wypłacie: **'..account.money, 'society_wyplata_gotowki')
...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	...
	for i=1, #(xPlayers) do 
		...
		local dutyTime = exports['heaven-misc']:getDutyTime(tonumber(xPlayer.source))
	...
	function(result)
		for k, row in pairs(result) do
			...
			if not alreadyInTable then
				...
				local dutyTime = exports['heaven-misc']:getDutyTime(identifier)
				table.insert(employees, {
					name = name,
