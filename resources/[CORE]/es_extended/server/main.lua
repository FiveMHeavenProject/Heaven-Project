SetMapName('SAN PROJECT')
SetGameType('HEAVEN PROJECT')
...
if Config.Multichar then
  newPlayer = newPlayer .. ', `firstname` = ?, `lastname` = ?, `dateofbirth` = ?, `sex` = ?, `height` = ?, `discord` = ?, `name` = ?, `created_at` = ?'
end
...
function createESXPlayer(identifier, playerId, data)
  ...
  if not Config.Multichar then
    ...
    MySQL.prepare(newPlayer, {json.encode(accounts), identifier, defaultGroup, discord, steamNick}, function()
      loadESXPlayer(identifier, playerId, true)
    end)

    TriggerEvent('hp-logs:sendLog', playerId, '[ZAŁOŻYŁ POSTAĆ]','\n**Gracz: **'..GetPlayerName(playerId)..'\n**Założył postać z discorda: **'..discord..'\n**Nick steam: **'..steamNick, 'esx_createchar')


  else
    local fIdentifiers = getIdentifiers(playerId)
    local discord = fIdentifiers.discord or "Brak"
    local steamNick = GetPlayerName(playerId)

    MySQL.prepare(newPlayer,
      {json.encode(accounts), identifier, defaultGroup, data.firstname, data.lastname, data.dateofbirth, data.sex, data.height, discord,steamNick, os.date('%Y-%m-%d')}, function()
        loadESXPlayer(identifier, playerId, true)
      end)
...
AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
  ...
  if not identifiers.steam then
    return deferrals.done('[HEAVEN PROJECT] Brak podpiętego steama do FiveM zalecany restart :)')
  else
    deferrals.done()
  end
end)
...
RegisterNetEvent('esx:tpCoord', function(coords, x,y,z)
  if coords then
