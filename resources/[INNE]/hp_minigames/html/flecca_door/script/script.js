...
const gameOver = () => {
	...
	$.post('https://cfx-nui-hp_minigames/minigameResult', JSON.stringify({ success: false }))
}
...
const gameFinish = () => {
	...
	finish = true
	progressBarStart('end', 2)
	$.post('https://cfx-nui-hp_minigames/minigameResult', JSON.stringify({ success: true }))
}
...
function moveCircleAndLines() {
	for (let i = 1; i < 16; i++) {
		document.getElementById('line' + i).style.top = getRndInteger(-40, -450) + 'px'
	}
