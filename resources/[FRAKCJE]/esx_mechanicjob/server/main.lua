...
exports.ox_inventory:RegisterStash('mechanic:stash', 'Skrytka mechaników', 200, 300000, false)
...
ESX.RegisterServerCallback('esx_mechanicjob:getVehicleInfo', function(source, cb, carId)
	local carBodyHP = GetVehicleBodyHealth(carId)
	local carEngineHP = GetVehicleEngineHealth(carId)
	local carPlate = GetVehicleNumberPlateText(carId)

	cb(carBodyHP, carEngineHP, carPlate)
end)
...
RegisterNetEvent("esx_mechanicjob:billing", function(ssn, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xPlayerSSN = xPlayer.get('ssn')
	...
	if mUser == false then xPlayer.showNotification("Nie ma takiego SSN w bazie") return end
	if ssn == xPlayerSSN then xPlayer.showNotification("Nie mozesz wystawic sobie sam faktury") return end

	TriggerEvent('esx_billing:sendMechanicBill', xPlayer.identifier , mUser.identifier, 'society_mechanic', "mechanik", amount)
end)

RegisterNetEvent("esx_mechanicjob:finishedFixkit", function() 
	local xPlayer = ESX.GetPlayerFromId(source)
