local weapons = {
	...
	[tostring(0)] = "Nie znaleziono",
	["przejechany"] = "Został przejechany",
}
...
AddEventHandler('esx:onPlayerDeath', function(data)
	data.zabity = source
	local usedWeapon = weapons[data.deathCause] or "Nie znaleziono"
	data.distance = data.distance or "Nie znaleziono"
	local killedByPlayer = data.killedByPlayer or "Nie znaleziono"
	local killerCoords = data.killerCoords
	
	local deathCoords = data.victimCoords or "Brak kordów"
	if killedByPlayer ~= "Nie znaleziono" then
		data.distance = "0"
		TriggerEvent('hp-logs:sendLog', source, '[GRACZ ZABITY]','\n\n**Gracz: **'..GetPlayerName(source)..'\n**Został zabity przez: **'..killedByPlayer..'\n**Z odległości: **'..data.distance..' metrów\n**Z broni: **'..usedWeapon..'\n**Kordy zabójcy: **'..data.killerCoords..'\n**Kordy gracza: **'..GetEntityCoords(GetPlayerPed(source)), 'zabicia')
	elseif not killerCoords then
		TriggerEvent('hp-logs:sendLog', source, '[GRACZ ZABITY]','\n\n**Gracz: **'..GetPlayerName(source)..'\n**Został zabity przez: **'..killedByPlayer..'\n**Z odległości: **'..data.distance..' metrów\n**Z broni: **'..usedWeapon..'\n**Kordy zabójcy: **Brak\n**Kordy gracza: **'..GetEntityCoords(GetPlayerPed(source)), 'zabicia')
